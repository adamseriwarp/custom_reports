...
def get_profit_by_lane_data(start_date, end_date, carriers, customers, lanes, shipment_type):
    ...
    if shipment_type == "Full Truckload":
        ...
        # - Direct (single row per order): use that row
        # - Multi-leg (multiple rows): use ONLY NO rows (YES duplicates revenue)
        query = f"""
        ...
        filtered_rows AS (
            ...
            WHERE {base_where}
              ...
              AND (
                -- Multi-leg: use NO rows only (avoid double counting)
                (orc.total_rows > 1 AND o.mainShipment = 'NO')
                -- Direct (single row): use whatever exists
                OR orc.total_rows = 1
              )
        )
        SELECT
        ...
        filtered_rows AS (
            ...
            WHERE {base_where}
              AND (
                ...
                -- LTL multi-leg: use NO rows only
                ...
                -- Other types: use mainShipment = YES
